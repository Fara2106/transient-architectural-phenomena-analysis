...
input_dir = "./perf_results_table_tlb_misses"

# Liste per memorizzare i dati
table_sizes = []  # Dimensioni delle tabelle
l1_misses = []    # Miss L1
l2_misses = []    # Miss L2
...
data = list(zip(table_sizes, l1_misses, l2_misses))
data.sort(key=lambda x: x[0])

table_sizes, l1_misses, l2_misses = zip(*data)
...
# Grafico L1
axs[0].plot(table_sizes, l1_misses, marker='o', linestyle='-', color='blue', label="L1 TLB Misses")
...
axs[0].set_title("L1 TLB Misses per Table Size")
axs[0].set_ylabel("TLB Misses")
axs[0].legend()
axs[0].grid(True)

# Grafico L2
axs[1].plot(table_sizes, l2_misses, marker='o', linestyle='-', color='orange', label="L2 TLB Misses")
...
axs[1].set_title("L2 TLB Misses per Table Size")
axs[1].set_xlabel("Table Size (rows)")
...
plt.tight_layout()
plt.savefig("tlb_misses_table_size_analysis.png")
plt.show()